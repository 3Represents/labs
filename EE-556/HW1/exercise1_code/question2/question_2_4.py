...
def main():
    ...
    rate_GD = (L - mu) / (L + mu)
    rate_GDmu = rate_GD
    theoretical_rate_gd = lambda k: rate_GD ** (k/2) * diffinit
    theoretical_rate_gd_str = lambda k: rate_GDmu ** k * diffinit_str
    ...
    plt.legend()
    plt.grid()
    plt.savefig("figs/fig_ex2_4_convergence_rates.pdf")
    ...
    a_GD = np.exp(intercept_GD[0])
    b_GD = np.exp(slope_GD[0])
    a_GDmu = np.exp(intercept_GDmu[0])
    b_GDmu = np.exp(slope_GDmu[0])

    # Coefficients corrected
    print("GD results: a_GD={:.6f}, b_GD={:.6f}".format(a_GD, b_GD))
    print("GD theoretical rate: a_GD={:.6f}, b_GD={:.6f}".format(diffinit, np.sqrt(rate_GD)))
    print("GD rate diff ={}".format(b_GD-np.sqrt(rate_GD)))
