...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(xs) =>
        val xsDec = xs.map(decoder.decode)
        if xsDec.forall(_.isDefined) then Some(xsDec.map(_.get)) else None
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(x) => if x.contains(name) then decoder.decode(x(name)) else None
      case _ => None
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform(user => Person(user._1, user._2))
