...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    def max(x: Tweet, y: Tweet): Tweet =
      if x.retweets > y.retweets then x else y
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists(wd => tw.text.contains(wd)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists(wd => tw.text.contains(wd)))
