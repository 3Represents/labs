...
object RecursiveLanguage {
  ...
  def eval(e: Expr, defs: DefEnv): Expr =
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => scrutinee match
        ...
        case _ => error(s"$scrutinee is not a list")
    ...
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) =>
        if headName == n || tailName == n then
          ...
          val fvs = freeVars(r)
          if fvs.contains(headName) then
            val headName1 = differentName(headName, fvs)
            val caseCons1 = alphaConvert(caseCons, headName, headName1)
            Match(subst(scrutinee, n, r), subst(caseEmpty, n, r), headName1, tailName, subst(caseCons1, n, r))
          else if fvs.contains(tailName) then
            val tailName1 = differentName(tailName, fvs)
            val caseCons1 = alphaConvert(caseCons, tailName, tailName1)
            Match(subst(scrutinee, n, r), subst(caseEmpty, n, r), headName, tailName1, subst(caseCons1, n, r))
  ...
  def show(e: Expr): String =
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) =>
        s"$scrutinee match\n" +
          s"  case Nil => ${show(caseEmpty)}\n" +
          s"  case $headName :: $tailName => ${show(caseCons)}"
  ...
  val definitions: DefEnv = Map[String, Expr](
    ...
    "map" -> Fun("ls", Fun("f",
      Match(N("ls"),
        Empty,
        "x", "xs",
        Cons(Call(N("f"), N("x")), Call(Call(N("map"), N("xs")), N("f")))))),
    ...
    "foldLeft" -> Fun("ls", Fun("acc", Fun("f",
      Match(N("ls"),
        N("acc"),
        "x", "xs",
        Call(Call(Call(N("foldLeft"), N("xs")), Call(Call(N("f"), N("acc")), N("x"))), N("f")))))),

    "foldRight" -> Fun("ls", Fun("z", Fun("fold",
      Match(N("ls"),
        N("z"),
        "x", "xs",
        Call(Call(N("fold"), N("x")), Call(Call(Call(N("foldRight"), N("xs")), N("z")), N("fold"))))))),
  )
